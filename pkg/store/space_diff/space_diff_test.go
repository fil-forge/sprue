	...
	for _, k := range storeKinds {
		t.Run(string(k), func(t *testing.T) {
			t.Run("puts a diff", func(t *testing.T) {
				...
				err := s.Put(t.Context(), space, cause, 1024, receiptAt)
			...
			t.Run("paginates results", func(t *testing.T) {
				...
				// first page of 2
				page, err := s.List(t.Context(), space, time.Time{}, spacediff.WithListLimit(2))
				require.NoError(t, err)
				require.Len(t, page.Results, 2)
				require.NotNil(t, page.Cursor)

				// second page using cursor
				page, err = s.List(t.Context(), space, time.Time{}, spacediff.WithListCursor(*page.Cursor))
				require.NoError(t, err)
				require.Len(t, page.Results, 3)
			})
			...
			t.Run("pages through diffs sharing one timestamp", func(t *testing.T) {
				...
				at := time.Now().UTC()
				for i := range 5 {
					require.NoError(t, s.Put(t.Context(), space, testutil.RandomCID(t), int64(i+1)*100, at))
				}
				...
				all, err := store.Collect(t.Context(), func(ctx context.Context, opts store.PaginationConfig) (store.Page[spacediff.DifferenceRecord], error) {
					...
					return s.List(ctx, space, time.Time{}, listOpts...)
				})
				require.NoError(t, err)
				require.Len(t, all, 5)
			...
			t.Run("supports negative deltas", func(t *testing.T) {
				...
				err := s.Put(t.Context(), space, cause, -512, time.Now().UTC())
				require.NoError(t, err)
...
func TestMemoryStoreConcurrentListOfUnseenSpace(t *testing.T) {
	...
	for range 16 {
		...
		go func() {
			defer wg.Done()
			page, err := s.List(t.Context(), testutil.RandomDID(t), time.Time{})
