...
func TestUCANConcludeHandler(t *testing.T) {
	...
	t.Run("receipt not in metadata", func(t *testing.T) {
		...
		concludeInv, err := cmducan.Conclude.Invoke(
			uploadService,
			uploadService.DID(),
			&cmducan.ConcludeArguments{Receipt: rcpt.Link()},
		...
		require.Equal(t, cmducan.ConclusionReceiptNotFoundErrorName, model.Name())
	})
