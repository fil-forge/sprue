...
func maybeAccept(
	...
	accReq := piriclient.AcceptRequest{
		...
		Digest: blob.Digest,
		Size:   blob.Size,
		// piriclient wraps this as promise.AwaitOK{Task: req.Put} (see
		// pkg/piriclient/client.go), and downstream consumers (guppy's
		// blobadd.go findInvocation) match invocations by Task().Link().
		// Passing putInv.Link() — the envelope link — would never match,
		// causing "missing invocation for task: ..." on upload.
